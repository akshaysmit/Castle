...
window.Basic_Shader = window.classes.Basic_Shader = class Basic_Shader extends Shader {
    ...
    update_GPU(g_state, model_transform, material, gpu=this.g_addrs, gl=this.gl) {
        ...
        gl.uniform1i(gpu.MY_VAR_loc, material.my_var);
    }
...
window.Phong_Shader = window.classes.Phong_Shader = class Phong_Shader extends Shader {
    ...
    material(color, properties) {
        ...
        return new class Material {
            constructor(shader, color=Color.of(0, 0, 0, 1), ambient=0, diffusivity=1, specularity=1, smoothness=40, normal_map=0) {
                // Assign defaults.
                Object.assign(this, {
                    shader,
                    color,
    ...
    shared_glsl_code() 
    {
        return `
            ...
            uniform bool GOURAUD, COLOR_NORMALS, USE_TEXTURE, normal_map;
    ...
    // ********* VERTEX SHADER *********
    //Modified to include normal mapping calculations
    vertex_glsl_code() {
        return `
            ...
            void main() {
                ...
                vec3 T = vec3(camera_model_transform * vec4(tangent, 1.0));
                T = normalize(T);
                vec3 B = cross(N, T);
                mat3 tbn = mat3(
                    T.x, B.x, N.x,
                ...
                vec3 vertex = vec3(camera_model_transform * vec4(object_space_pos, 1.0));
                tangent_light_dir = (L[0] - vertex)*tbn;
                tangent_eye_dir = (-vertex)*tbn;
    ...
    fragment_glsl_code() {
        return `
            ...
            void main() {
                ...
                //change normal if normal mapping is used
                vec3 final_N;

                if (normal_map) {
                    final_N = normalize(2.0 * (texture2D(normal_texture, f_tex_coord).rgb - 0.5));
                }
                else {
                    final_N = N;
                }
                ...
                gl_FragColor.xyz += phong_model_lights( final_N );
            }`;
    }
    ...
    update_GPU(g_state, model_transform, material, gpu=this.g_addrs, gl=this.gl) {
        ...
        gl.uniform1i( gpu.normal_map_loc,  material.normal_map);
        ...
        //Send normal map to shader file
        if (material.normal_map) {
            gpu.shader_attributes["tangent"].enabled = true;
            gl.activeTexture(gl.TEXTURE1);
            gl.bindTexture(gl.TEXTURE_2D, material.normal_texture.id);
            gl.uniform1i(gpu.normal_texture_loc, 1);
        }
        else {
            gpu.shader_attributes["tangent"].enabled = false;
        }
